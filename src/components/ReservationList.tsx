...
// Rendering issue on next page fast clicking (possible solution: debounce)
...
const ReservationList: React.FC<{
  ...
  changedReservation?: ReservationData;
}> = ({ onOpen, changedReservation }) => {
  ...
  useEffect(() => {
    ...
    setRows((prevRows) => {
      const rowIndex = prevRows.findIndex(
        (row) => row.id === changedReservation.id
      );
      if (rowIndex === -1) return prevRows;
      const updatedRows = [...prevRows];
      updatedRows[rowIndex] = { ...prevRows[rowIndex], ...changedReservation };
      return updatedRows;
    });
  }, [changedReservation]);
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            value={perPage}
            onChange={(e) => {
              setPage(1); // This resents to first page but if we store all the already fetched elements there's no need to go back as we can work what to display
