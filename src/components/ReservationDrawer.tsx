...
// if fn is used more than once make it util function and unit test it
...
const ReservationDrawer: React.FC<{
  open: boolean;
  id: string | null;
  onClose: (reservation?: ReservationData) => void;
}> = ({ open, id, onClose }) => {
  const [data, setData] = useState<ReservationData | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null); // structure this as an object of erros that refers to the invalid inputs
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (!data) return;
    // another early return is that when we have ANY error
    setFormError(null);
    setLoading(true);
